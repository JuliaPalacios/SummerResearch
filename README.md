...
| June 19   | [Statistics for Phylogenetic Trees](http://pages.stat.wisc.edu/~larget/phylogeny/holmes-thPopBio-2003.pdf) | All | Kyle: In this paper, Susan Holmes writes about the challenges inherent in solving the problems of inference on tree space, and addresses some of the possible ways to deal with these problems. Estimations for trees have been contested for a great amount of time, as trees provide an excellent unit of summary information. Phylogenetic trees have both a discreteness and a complexity that justify recourse to a series of tools from statistics and geometry. Basic information is coded into distances, namely the expected number of mutations. Distance-based methods are semiparametric, on the spectrum of nonparametric to parametric models. |
| June 19   | [Computational Tools for Evaluating Phylogenetic and Hierarchical Clustering Trees](https://arxiv.org/pdf/1006.1015.pdf) | All (Kuhan will present) | Kyle: This paper presents an implementation of the geodesic distance between trees developed by Billera, Holmes, and Vogtmann (2001). It also describes tools for estimating phylogenetic trees from Data. Holmes evaluates trees by comparing two hierarchical clusterings through creating a weighted matching score from the matrix of matchings and by implementing the polynomial time geodesic path algorithm. The goal is to get the geodesic distance between two trees. Very interesting read.  |
| June 26 | [Feretti et al., 2017] | Alan Aw presenting | To fill in. |
| August 2 | [Mapping the Shapes of Phylogenetic Trees from Human and Zoonotic RNA Viruses](http://journals.plos.org/plosone/article/file?id=10.1371/journal.pone.0078122&type=printable) | All (Kuhan will present) | Kuhan: This paper introduces a new procedure to study phylogenetic tree shapes based on the ‘kernel method’, which reduces the number of dimensions to ‘map complex objects into a statistical convenient space.’ This method outperforms 9 classical tree balance statistics in differentiating and classifying phylogenies. The kernel function they created extracts all subset trees that are the common features of two phylogenetic trees, where a subset tree is a contiguous collection of descendants of a specific node (but unlike a subtree doesn’t necessarily include all descendants). This function is superior to the Robinson-Foulds metric as it incorporates branch lengths between matching subset trees along with numbers and kinds of taxa with the RF metric already compares. Decided to analyze rooted trees by selecting an outgroup sequence based on previous literature, and their kernel analysis was robust to choice of outgroup data, and capped the max tree size at 200 as to not have size heavily influence their comparisons. They ladderized trees to help compare them, meaning rotating branches around every node and rotated cherries so that branches with more descendants were always on the same side. Results showed that HIV and HCV phylogenies were quite distinct from other RNA viruses.  Viruses that can establish persistent chronic infections are unambiguously separated from other RNA viruses in the space defined by the tree kernel. |
| August 2 | [Computing Geodesic Distances in Tree Space](https://arxiv.org/pdf/0903.0696.pdf) | Kyle will present | To fill in. | 
